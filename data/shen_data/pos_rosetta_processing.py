...
df = pd.read_csv('hiv_benchmarks/data/shen_data/rosetta_lr/output.csv')
data = process_dataframe(df)
data.to_csv('hiv_benchmarks/data/shen_data/rosetta_lr/pi_monomer_rosetta.csv', index=False)


data1 = data.iloc[1:].reset_index(drop=True)

data2 = pd.read_csv('hiv_benchmarks/data/shen_data/pi_sequences_classification.csv')
...
dataset.to_csv('/hiv_benchmarks/data/shen_data/rosetta_lr/shen_rosetta_modeled_classification.csv', index=False) # Data for LR models
...
def LogRegClass(df, drug, out, feature_selection = False, c_mi=0.8):
    ...
    logging.info(f"Starting training for: {drug}")
    ...
    x_train = train_df.drop(["SeqID", "FPV", "ATV","IDV", "LPV", "NFV", "SQV", "TPV", "DRV"], axis = 1)
    y_train = train_df[drug].values

    x_test = test_df.drop(["SeqID", "FPV", "ATV","IDV", "LPV", "NFV", "SQV", "TPV", "DRV"], axis = 1)
...
os.chdir('hiv_pr_benchmarks/data/shen_data/')
data = pd.read_csv('shen_rosetta_modeled_classification.csv')
